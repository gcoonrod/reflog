...
export function findInviteByEmail(db: D1Database, email: string) {
  return db
    .prepare("SELECT * FROM invites WHERE email = ?")
    .bind(email);
}

export function findInviteByToken(db: D1Database, token: string) {
  return db
...
export function countConsumedInvites(db: D1Database) {
  return db.prepare(
    "SELECT COUNT(*) as count FROM invites WHERE status = 'consumed'"
  );
}
...
export function createWaitlistEntry(
  ...
  return db
    .prepare("INSERT INTO waitlist (id, email, consent) VALUES (?, ?, ?)")
    .bind(id, email, consent);
}

export function findWaitlistByEmail(db: D1Database, email: string) {
  return db
...
export function getBetaConfig(db: D1Database, key: string) {
  return db
    .prepare("SELECT value FROM beta_config WHERE key = ?")
    .bind(key);
}
